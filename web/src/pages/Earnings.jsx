...
import { Stat, Card, Badge, EmptyState } from '../components/ui.jsx';
import { IconPackage } from '../components/icons.jsx';
...
export default function Earnings() {
  ...
  useEffect(() => {
    api.earnings().then((d) => { setPayouts(d.payouts); setTotals(d.totals); }).catch(() => {});
  }, []);
          ...
          <Card className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {payouts.map((p) => (
                    <tr key={p.job_code} className="border-b last:border-0" style={{ borderColor: 'var(--border-subtle)' }}>
                      <td className="px-5 py-3 font-mono text-xs text-ink-secondary">{p.job_code}</td>
